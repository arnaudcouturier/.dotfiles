...
| `dotfiles/desktop/ghostty/config` | Supersede rule | `home-arch/` Ghostty wins on Arch only; shared DMS config untouched on disk and sole-sourced on Fedora. |
...
- Blanket collision `die` for known overrides: rejected. Ghostty supersedes on Arch by selection; `hypr/input.lua` is an explicit Arch exclude with an equivalent option in the Arch override — never a verifier that contradicts the selected stow paths.
