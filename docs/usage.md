...
| `npm "package"` | Fedora | global install, always `--ignore-scripts` |
| `npm-scripts "package"` | Fedora | exemption when the postinstall *is* the install |
...
pinned RPM > AppImage > npm) and `repo` over `aur`. Flatpak entries need
